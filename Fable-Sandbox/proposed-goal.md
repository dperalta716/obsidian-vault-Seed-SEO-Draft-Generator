...
14. **Run-session clarifications from David (2026-06-10, during execution).** (a) The orchestrating session stays on **Fable for the entire run** — no model switching; Opus is used only for the spawned drafting/reviewing/grading subagents per handoff §3.3. (b) Everything being modified is **already in the sandbox**: the generator lives at `Fable-Sandbox/skills/generate-ds01-draft-fable/SKILL.md` and the four review stages + chain runner at `Fable-Sandbox/commands/*-fable.md`. No original under `.claude/` is ever edited. (c) Scope note: this is standard content-production work about the gut microbiome and probiotics — nothing sensitive.
