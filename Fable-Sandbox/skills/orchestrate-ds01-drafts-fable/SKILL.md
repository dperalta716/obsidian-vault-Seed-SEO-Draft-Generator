...
description: >
  ...
  /generate-ds01-draft-fable on ALL keywords in parallel with pre-assigned folder numbers to
  prevent collisions. After user checkpoint, runs /full-draft-review-auto-ds01-fable on each
...
1. Read the `Fable-Sandbox/Generated-Drafts/` directory listing
2. Extract all 3-digit numeric prefixes
3. Find the highest number
4. Assign the next N sequential numbers (one per brief)
5. Pre-create all N folders in `Fable-Sandbox/Generated-Drafts/`:
...
mkdir -p "Fable-Sandbox/Generated-Drafts/095-how-to-test-gut-health/competitors"
mkdir -p "Fable-Sandbox/Generated-Drafts/096-best-vegetables-for-gut-health/competitors"
mkdir -p "Fable-Sandbox/Generated-Drafts/097-best-mushroom-for-gut-health/competitors"
mkdir -p "Fable-Sandbox/Generated-Drafts/098-is-green-tea-good-for-gut-health/competitors"
mkdir -p "Fable-Sandbox/Generated-Drafts/099-are-bananas-good-for-gut-health/competitors"
mkdir -p "Fable-Sandbox/Generated-Drafts/100-best-bread-for-gut-health/competitors"
```
...
4. **The skill to invoke**: `/generate-ds01-draft-fable [keyword]`
...
Your output folder is: Fable-Sandbox/Generated-Drafts/[NNN]-[slug]/
...
Invoke /generate-ds01-draft-fable [keyword] and follow the full workflow.
...
- Success: `[keyword] — draft complete (Fable-Sandbox/Generated-Drafts/[NNN]-[slug]/)`
...
2. **The skill to invoke**: `/full-draft-review-auto-ds01-fable [path-to-v1-file]`

**Template for each subagent prompt:**
```
Run the full DS-01 draft review pipeline on this article:
[path to v1 draft file, e.g., Fable-Sandbox/Generated-Drafts/095-how-to-test-gut-health/v1-how-to-test-gut-health-claude-draft.md]

Invoke /full-draft-review-auto-ds01-fable with the file path above.
Run all 4 steps: Seed Perspective → Quality → Sources → Claims. (SANDBOX: no Google Docs upload — pipeline ends at v5.)
Report back with the completion summary and the v5 file path.
```

## Step 9: Monitor & Final Report

Same pattern as Step 6. Report each completion with the v5 file path.

Final summary:
```
Full Review Complete
====================
[x] 095 - how to test gut health → [v5 file path]
[x] 096 - best vegetables for gut health → [v5 file path]
[x] 097 - best mushroom for gut health → [v5 file path]
[x] 098 - is green tea good for gut health → [v5 file path]
[x] 100 - best bread for gut health → [v5 file path]

5 drafts generated and reviewed. (No upload in sandbox.)
