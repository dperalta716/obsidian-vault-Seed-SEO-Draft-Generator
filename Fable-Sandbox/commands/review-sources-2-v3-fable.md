...
- `/review-sources-2-v3` - Reviews sources in the most recent draft in /Fable-Sandbox/Generated-Drafts/ or /Phase 1 Draft Revsions/
...
Same as v2:
- If no argument: Find most recent versioned file in `/Fable-Sandbox/Generated-Drafts/` or `/Phase 1 Draft Revsions/`
...
#### 3f. Source Fitness Check (record + flag, never auto-replace)

For each citation that passed integrity (3e = ✅ VALID), record three attributes from the metadata/abstract already retrieved — study type, population/model, year — then flag (do NOT auto-fix) any that are:

- ⚠️ **Population mismatch** — general-audience claim, but clinical/pediatric/animal/in-vitro source, AND a healthy-adult source plausibly exists.
- ⚠️ **Scope drift** — source subject broader than or adjacent to the claim, not a direct match.
- ⚠️ **Stale** — pre-2019 and not marked seminal.
- ⚠️ **Reputability** — non-peer-reviewed page (excluding NIH/CDC/FDA).

These are advisory quality flags, separate from integrity fixes. Citations that are real and correctly attributed but weak in fitness stay in the article — they surface for human/SciCare review, they are NOT auto-replaced (auto-swapping risks replacing a correct citation with a worse-matched one).

**Append the flags to the END of the new version file** (so they travel with the draft into Google Docs, not just the terminal report):
...
- **[Author Year]** — ⚠️ population mismatch: claim is general-audience, source is an IBS-patient study.
- **[Author Year]** — ⚠️ scope drift: cited for the category, study is product/strain-specific.
- **[Author Year]** — ⚠️ stale (2014); not marked seminal.
```

_If no flags, write instead: "Source fitness: all cited sources passed population, scope, recency, and reputability checks."_
