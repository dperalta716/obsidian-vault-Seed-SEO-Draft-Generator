...
# Create Gmail draft
# Usage: ./draft-message.sh "user@example.com" "to@example.com" "Subject" "Body" [cc] [bcc] [thread_id] [in_reply_to] [references] [from_name]
...
if [[ -n "$THREAD_ID" ]]; then
  REQUEST_BODY=$(jq -n \
    ...
    '{message: {raw: $raw, threadId: $threadId}}')
else
  REQUEST_BODY=$(jq -n \
    --arg raw "$ENCODED_MESSAGE" \
    '{message: {raw: $raw}}')
